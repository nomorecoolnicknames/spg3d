...
for (const car of process.argv.slice(2)) {
  await page.goto(`http://localhost:${port}/?screen=garage&car=${car}&q=low&pose=0`, { timeout: 120000 });
  await page.waitForFunction(() => window.__spg && window.__spg.ready, null, { timeout: 180000 });
  await page.waitForTimeout(1500);
  console.log(JSON.stringify(await page.evaluate(() => window.__spg.knobs.showcaseProbe?.())));
}
await browser.close();
process.exit(0);
