...
# Outputs src/assets/cars/<model>.glb (LOD0-2, 1024 atlas) and <model>-hd.glb (LOD0, 2048 atlas).
...
for i in $(seq ${FROM:-0} $((N - 1))); do
  ...
  node scripts/atlas-cars.mjs "$WORK/rig/$MODEL.rig.glb" "src/assets/cars/$MODEL.glb" "$(node -e "console.log(JSON.stringify({...JSON.parse(process.argv[1]), atlas: 1024, lods: [0,1,2], debugDir: '$WORK/rig'}))" "$SPEC")" | grep ATLAS_OK
  node scripts/atlas-cars.mjs "$WORK/rig/$MODEL.rig.glb" "src/assets/cars/$MODEL-hd.glb" "$(node -e "console.log(JSON.stringify({...JSON.parse(process.argv[1]), atlas: 2048, lods: [0]}))" "$SPEC")" | grep ATLAS_OK
done
