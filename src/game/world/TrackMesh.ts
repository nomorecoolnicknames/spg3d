...
export function buildTrackMesh(track: TrackData, quality: { shadows: boolean; low?: boolean }): TrackMesh {
  ...
  const roadMat = createRoadMaterial(env, { wetness: env.wet ? 0.85 : spec.theme === 'snow' ? 0.5 : 0, lineColor: spec.theme === 'snow' ? '#c9d6e4' : '#d8d8d8', low: !!quality.low, lamps: !quality.low && env.headlights });
  ...
  // city streets: a see-through town railing on a plinth (no neon); otherwise concrete blocks with hazard stripes
  const railing = spec.theme === 'city';
  ...
  // reflector posts outside the city
  if (spec.theme !== 'city') {
